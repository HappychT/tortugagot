...
public enum GOTFaction {
    ...
    public int[] calculateFullControlZoneWorldBorders() {
        ...
        int zMax = 0;
        boolean first = true;
        for (GOTControlZone zone : this.controlZones) {
    ...
    public void checkAlignmentAchievements(EntityPlayer entityplayer, float alignment) {
        ...
        for (GOTFactionRank rank : this.ranksSortedDescending) {
    ...
    public boolean isFactionDimension(World world) {
        return world.provider instanceof GOTWorldProvider && ((GOTWorldProvider) world.provider).getGOTDimension() == this.factionDimension;
    }
