...
public class AttackHandler {
    ...
    public void onEntityConstructing(EntityEvent.EntityConstructing event) {
        if (event.entity instanceof EntityPlayer) {
            ...
            if (ExtendedPlayer.get((EntityPlayer) event.entity) == null) {
                // Если ваш ExtendedPlayer для стамины тоже нужно регистрировать здесь
    ...
    public void onLivingHurt(LivingHurtEvent event) {
        ...
        if (heldItemType == GOTRegistry.stunningHammer) {
            if (target.isPotionActive(Potion.moveSlowdown.id)) {
                event.ammount += 2.0F;
            }
        } else if (heldItemType == GOTRegistry.decoyClaymore) {
        ...
        } if (heldItemType == GOTRegistry.nightkingSword) {
