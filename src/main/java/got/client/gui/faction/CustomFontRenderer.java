...
public class CustomFontRenderer {
		...
		return (UnicodeFont) cache.computeIfAbsent(font.font + size, (o) -> {
			...
			try {
				InputStream uf = Minecraft.getMinecraft().getResourceManager()
						.getResource(new ResourceLocation("got", "fonts/" + font.font + ".ttf")).getInputStream();
	...
	public static void drawStringNew2(String string, float x, float y, String fontName, int fontSize, int color) {
		drawStringWithMaxWidthNew(string, x, y, -1, fontName, fontSize, color);
	}

	public static void drawStringWithMaxWidthNew(String string, float x, float y, int w, final String fontName, final int fontSize, int customColor) {
		final CustomFont font = new CustomFont(fontName, fontSize);
		drawStringWithMaxWidthNew(string, x, y, w, font, customColor);
	}
	...
	public static void drawStringNew(String string, float x, float y, CustomFont font, int color) {
		drawStringWithMaxWidthNew(string, x, y, -1, font, color);
	}
	public static void drawStringWithMaxWidthNew(String string, float x, float y, int w, final CustomFont font, int customColor) {
		final float guiScale = CustomFontRenderer.fontScale * 2.0f;
		final float rscale = 1.0f / guiScale;
		x *= (int) guiScale;
		y *= (int) guiScale;
		...
		boolean useCustomColor = (customColor != -1);

		if (!string.startsWith("§") && !useCustomColor) {
			string = "§f" + string;
		}
				...
				if (s1.length() != 0) {
					...
					if (useCustomColor) {
						for (final String s3 : s1.split(" ")) {
							...
							uf.drawString((float) x, (float) y, t, new org.newdawn.slick.Color(customColor >>> 16 & 0xFF, customColor >>> 8 & 0xFF, customColor & 0xFF, customColor >>> 24 & 0xFF));
							width += uf.getWidth(t);
							x += uf.getWidth(t);
						}
					} else {
