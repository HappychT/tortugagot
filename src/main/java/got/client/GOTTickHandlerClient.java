...
public class GOTTickHandlerClient {
        ...
        if (worldClient != null && entityClientPlayerMP != null) {
                ...
                if (itemstack != null && !itemstack.hasDisplayName()) {
                    ...
                    if (!enchants.isEmpty()) {
                        ...
                        if (itemstack.hasDisplayName()) {
                            this.highlightedItemstackName = itemstack.getDisplayName();
                        } else {
                            this.highlightedItemstackName = null;
                        }
            ...
            if (event.type == RenderGameOverlayEvent.ElementType.HELMET) {
                ...
                if (this.mistFactor > 0.0F) {
                    renderOverlay(null, this.mistFactor * 0.75F, mc, mistOverlay);
                }
                if (this.frostTick > 0) {
                    ...
                    renderOverlay(null, frostAlpha * 0.3F, mc, frostOverlay);
                }
                if (this.burnTick > 0) {
                    renderOverlay(null, this.burnTick / 40.0F * 0.6F, mc, burnOverlay);
                }
                if (this.wightLookTick > 0) {
                    renderOverlay(null, this.wightLookTick / 100.0F * 0.95F, mc, wightOverlay);
                }
            }
            ...
            if (event.type == RenderGameOverlayEvent.ElementType.HEALTH && entityClientPlayerMP.isPotionActive(GOTEffects.killingPoison) && !entityClientPlayerMP.isPotionActive(Potion.poison)) {
    ...
    public void onRenderTick(TickEvent.RenderTickEvent event) {
        ...
        if (event.phase == TickEvent.Phase.END) {
            if (entityplayer != null && worldClient != null) {
                ...
                if (entityplayer.dimension == GOTDimension.GAME_OF_THRONES.dimensionID && minecraft.currentScreen == null && this.newDate > 0) {
                    int halfMaxDate = 100;
                    float alpha = 0.0F;
                    if (this.newDate > halfMaxDate) {
                        alpha = (200 - this.newDate) / halfMaxDate;
                    } else {
                        alpha = this.newDate / halfMaxDate;
                    }
                ...
                if (GOTConfig.displayMusicTrack && minecraft.currentScreen == null && this.lastTrack != null && this.musicTrackTick > 0) {
    ...
    public void renderAlignment(Minecraft mc, float f) {
        ...
        int width = resolution.getScaledWidth();
        resolution.getScaledHeight();
        this.alignmentXBase = width / 2 + GOTConfig.alignmentXOffset;
