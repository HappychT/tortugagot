...
public class GOTNetHandlerPlayServer extends NetHandlerPlayServer {
    ...
    public void processMountControl(GOTPacketMountControl packet) {
        ...
        if (mount != null && mount != this.playerEntity && mount.riddenByEntity == this.playerEntity && GOTMountFunctions.isMountControllable(mount)) {
            ...
            if (distSq - speedSq > 150.0 && (!server.isSinglePlayer() || !server.getServerOwner().equals(this.playerEntity.getCommandSenderName()))) {
                FMLLog.warning(mount.getCommandSenderName() + " (mount of " + this.playerEntity.getCommandSenderName() + ") moved too quickly! " + (distSq - speedSq));
    ...
    public void setBlockReach() {
        ...
        this.playerEntity.theItemInWorldManager.setBlockReachDistance(reach *= GOTWeaponStats.getMeleeReachFactor(this.playerEntity.getHeldItem()));
    }
