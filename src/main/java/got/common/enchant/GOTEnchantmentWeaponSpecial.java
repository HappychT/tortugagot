...
//            return StatCollector.translateToLocalFormatted("got.enchant." + this.enchantName + ".desc.melee");
    ...
    public boolean isCompatibleWith(GOTEnchantment other) {
        if (!this.compatibleOtherSpecial && other instanceof GOTEnchantmentWeaponSpecial && !((GOTEnchantmentWeaponSpecial) other).compatibleOtherSpecial)
            return false;
    ...
    public static void doChillAttack(EntityLivingBase entity) {
        ...
        int duration = 5;
        entity.addPotionEffect(new PotionEffect(GOTEffects.freeze.id, duration * 20));
