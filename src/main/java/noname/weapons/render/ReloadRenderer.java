...
public class ReloadRenderer extends Gui {
    ...
    public void render() {
        ...
        if (player.ridingEntity == null || !(player.ridingEntity instanceof EntityCatapult)) {
            return;
        }

        EntityCatapult catapult = (EntityCatapult) player.ridingEntity;
        ScaledResolution scaledResolution = new ScaledResolution(mc, mc.displayWidth, mc.displayHeight);
        int screenWidth = scaledResolution.getScaledWidth();
        int screenHeight = scaledResolution.getScaledHeight();

        float rangePower = catapult.getDataWatcher().getWatchableObjectFloat(28);
