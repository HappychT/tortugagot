...
public class ReloadTribushetRender extends Gui {
    ...
    public void render() {
        ...
        if (player.ridingEntity == null || !(player.ridingEntity instanceof EntityTribushet)) {
            return;
        }

        EntityTribushet tribushet = (EntityTribushet) player.ridingEntity;
        ScaledResolution scaledResolution = new ScaledResolution(mc, mc.displayWidth, mc.displayHeight);
        int screenWidth = scaledResolution.getScaledWidth();
        int screenHeight = scaledResolution.getScaledHeight();

        float rangePower = tribushet.getDataWatcher().getWatchableObjectFloat(28);
        ...
        boolean isReloading = (tribushet.getDataWatcher().getWatchableObjectByte(17) & 1) != 0;
        int reloadTimer = tribushet.getDataWatcher().getWatchableObjectInt(18);

        if (!isReloading) return;

        int maxReloadTime = (int) WeaponsConfig.reloadTimeTribushet;
